...
async def handle_shutdown_query(_, callback_query):
    if callback_query.data == "shutdown_yes":
        ...
        await bot_management_panel(_, callback_query)
...
async def start(client, message):
    ...
    update_user_info_from_telegram(message.from_user)
...
def update_user_info_from_telegram(user):
    ...
    # Initialize empty dictionary for existing info
    existing_info = {}
    ...
    if os.path.exists(USER_INFO_FILE):
        try:
            with open(USER_INFO_FILE, 'r') as f:
                ...
                if os.path.getsize(USER_INFO_FILE) > 0:
    ...
    existing_info[str(user.id)] = user_info
...
async def admin_panel(client, message):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("🤖 Manage Bot", callback_data="bot_management"),  # New button
...
@Mbot.on_callback_query(filters.regex(r"view_profile"))
async def view_profile(client, callback_query):
    ...
    profile_text = (
        ...
        f"📞 <b>Phone Number:</b> {user.phone_number if hasattr(user, 'phone_number') else 'Not visible'}\n"
        f"🌍 <b>Language:</b> {LANGUAGES.get(user_lang, 'Not set')}"
    )
    ...
    try:
        ...
        if photos.total_count > 0:
            ...
            photo = await client.download_media(photos.photos[0][-1].file_id)
    ...
    # If no photo or error, just send the text
    await callback_query.message.edit_text(
        profile_text,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 Back", callback_data="settings")]
...
async def view_profile(client, callback_query):
    ...
    # Format the profile information using translations
    profile_text = (
        f"👤 <b>{responses.get('profile_title', 'Your Profile Information')}</b>\n\n"
        f"{responses.get('user_id', '🆔 User ID')}: <code>{user.id}</code>\n"
        f"{responses.get('first_name', '📛 First Name')}: {user.first_name}\n"
        f"{responses.get('last_name', '📛 Last Name')}: {user.last_name or responses.get('not_set', 'Not set')}\n"
        f"{responses.get('username', '🌐 Username')}: @{user.username if user.username else responses.get('not_set', 'Not set')}\n"
        f"{responses.get('phone_number', '📞 Phone Number')}: {user.phone_number if hasattr(user, 'phone_number') else responses.get('not_visible', 'Not visible')}\n"
        f"{responses.get('language', '🌍 Language')}: {LANGUAGES.get(user_lang, responses.get('not_set', 'Not set'))}"
    )
    ...
    User_Profile = {
    "en": {
        "profile_title": "👤 Your Profile Information",
        "user_id":       "🆔 User ID",
    ...
    "fa": {
        "profile_title": "👤 اطلاعات پروفایل شما",
        ...
        "language":      "🌍 زبان",
        "not_set":       "تنظیم نشده",
        "not_visible":   "قابل مشاهده نیست"
    },

    "hi": {
        "profile_title": "👤 आपकी प्रोफ़ाइल जानकारी",
        ...
        "not_visible":   "दृश्य नहीं"
    },

    "ru": {
        "profile_title": "👤 Информация вашего профиля",
    ...
    "es": {
        "profile_title": "👤 Información de tu perfil",
...
@Mbot.on_callback_query(filters.regex(r"bot_management"))
async def bot_management_panel(client, callback_query):
    await callback_query.answer()
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("🗑️ Delete Session File", callback_data="delete_session_file"),
        ],
        [
            InlineKeyboardButton("🔙 Back", callback_data="admin")
        ]
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await callback_query.message.edit_text("🤖 Bot Management:\nChoose an action:", reply_markup=reply_markup)

@Mbot.on_callback_query(filters.regex(r"delete_session_file"))
async def delete_session_file_callback(client, callback_query):
    await callback_query.answer()
    
    keyboard = [
        [
            InlineKeyboardButton("🟢 Yes", callback_data="confirm_delete_session"),
            InlineKeyboardButton("🔴 No", callback_data="bot_management")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await callback_query.message.edit_text(
        "⚠️ Are you sure you want to delete the ':memory:.session' file?\n\n"
        "This will log out the bot and require re-login on next start.",
        reply_markup=reply_markup
    )

@Mbot.on_callback_query(filters.regex(r"confirm_delete_session"))
async def confirm_delete_session(client, callback_query):
    session_file = ":memory:.session"
    try:
        if os.path.exists(session_file):
            os.remove(session_file)
            await callback_query.answer("✅ Session file deleted successfully!", show_alert=True)
        else:
            await callback_query.answer("ℹ️ Session file not found", show_alert=True)
    except Exception as e:
        await callback_query.answer(f"❌ Error deleting session file: {str(e)}", show_alert=True)
    
    # Go back to bot management panel
    await bot_management_panel(client, callback_query)